...
class Knowledge extends CI_Controller
{
    ...
    public function get_data()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            $filter = new stdClass();
            $filter->select = 'id,image,title_en,title_th,title_ae,short_description_en,short_description_th,short_description_ae,description_en,description_th,description_ae,created_date,status';
            $data = $this->Knowledge_model->search($filter);
    ...
    public function get_datadetails()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $data = $this->Review_model->search($filter);
            $this->response_library->responseJSON($code['200']['CODE'], $code['200']['MESSAGE'], $data);
        }
