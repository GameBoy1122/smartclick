...
class Administrator_group_model extends CI_Model
{
    /** ========================================================================================= **
     **           SETTING
     ** ========================================================================================= **/
    ...
    private $table_name = "administrator_group";
    private $table_primary_key = "administrator_group_id";
    ...
    function validate($validate = null)
    {
        $response = array(
            ...
            "ERROR_STATUS_ISNULL"           => array("code" => "0x000M-AG001", "message" => "Server response failed. Request process incomplete status is required."),
            "ERROR_NAME_ISNULL"             => array("code" => "0x000M-AG002", "message" => "Server response failed. Request process incomplete name is required."),
            "ERROR_NAME_DUPLICATE"          => array("code" => "0x000M-AG011", "message" => "Server response failed. Request process incomplete name are already in used."),
        );

        /** VALIDATE NOT NULL */
        if(!isset($validate->status)        || $validate->status == "")         return $response["ERROR_STATUS_ISNULL"];
        if(!isset($validate->name)          || $validate->name == "")           return $response["ERROR_NAME_ISNULL"];
        /** VALIDATE EXISTS */
        $search_filter = new stdClass();
        $search_filter->where["name"] = $validate->name;
        ...
        if(count($result) > 0) return $response["ERROR_NAME_DUPLICATE"];
