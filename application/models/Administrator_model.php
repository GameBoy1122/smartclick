...
class administrator_model extends CI_Model
{
    ...
    private $table_primary_key = "administrator_id";
    ...
    {
        $response = array(
            "SUCCESS"                             => array("code" => "0x0000-00000", "message" => "Server response success. Request process complete with no error."),
            "ERROR_ADMINISTRATOR_GROUP_ID_ISNULL" => array("code" => "0x000M-A0001", "message" => "Server response failed. Request process incomplete administrator group is required."),
            "ERROR_STATUS_ISNULL"                 => array("code" => "0x000M-A0002", "message" => "Server response failed. Request process incomplete status is required."),
            "ERROR_USERNAME_ISNULL"               => array("code" => "0x000M-A0003", "message" => "Server response failed. Request process incomplete username is required."),
            "ERROR_EMAIL_ISNULL"                  => array("code" => "0x000M-A0004", "message" => "Server response failed. Request process incomplete email is required."),
            "ERROR_EMAIL_PASSWORD"                => array("code" => "0x000M-A0005", "message" => "Server response failed. Request process incomplete password is required."),
            "ERROR_USERNAME_DUPLICATE"            => array("code" => "0x000M-A0011", "message" => "Server response failed. Request process incomplete username are already in used."),
            "ERROR_EMAIL_DUPLICATE"               => array("code" => "0x000M-A0012", "message" => "Server response failed. Request process incomplete email are already in used."),
        );

        /** VALIDATE NOT NULL */
        if(!isset($validate->administrator_group_id) || $validate->administrator_group_id == "")  return $response["ERROR_ADMINISTRATOR_GROUP_ID_ISNULL"];
        if(!isset($validate->status)                 || $validate->status == "")                  return $response["ERROR_STATUS_ISNULL"];
        if(!isset($validate->username)               || $validate->username == "")                return $response["ERROR_USERNAME_ISNULL"];
        if(!isset($validate->email)                  || $validate->email == "")                   return $response["ERROR_EMAIL_ISNULL"];
        if(!isset($validate->password)               || $validate->password == "")                return $response["ERROR_EMAIL_PASSWORD"];
        /** VALIDATE EXISTS */
        $search_filter = new stdClass();
        $search_filter->where["username"] = $validate->username;
        $search_filter->where_in["status"] = array("ACTIVATE","SUSPEND");
        if(isset($validate->{$this->table_primary_key})) $search_filter->where[$this->table_primary_key." !="] = $validate->{$this->table_primary_key};
        $result = $this->search($search_filter);
        if(count($result) > 0) return $response["ERROR_USERNAME_DUPLICATE"];

        $search_filter = new stdClass();
        $search_filter->where["email"] = $validate->email;
        $search_filter->where_in["status"] = array("ACTIVATE","SUSPEND");
        if(isset($validate->{$this->table_primary_key})) $search_filter->where[$this->table_primary_key." !="] = $validate->{$this->table_primary_key};
        $result = $this->search($search_filter);
        if(count($result) > 0) return $response["ERROR_EMAIL_DUPLICATE"];
    ...
    function getAdministrator($administrator_id)
    {
        ...
        $administrator = $this->search($model_filter);
