...
class Administrator_permission_model extends CI_Model
{
    /** ========================================================================================= **
     **           SETTING
     ** ========================================================================================= **/
    ...
    private $table_name = "administrator_permission";
    private $table_primary_key = "administrator_permission_id";
     ...
     ** ========================================================================================= **/
    function getAdministratorPermissionsByAdministratorId($administrator_id)
    {
        $model_filter = new stdClass();
        $model_filter->where["administrator.administrator_id"] = $administrator_id;
        $model_filter->join = array(
            "administrator" => "administrator_id",
            "administrator_permission_key" => "administrator_permission_key_id"
        );
        $administrator_permissions = $this->search($model_filter);

        $response = array();
        foreach($administrator_permissions as $administrator_permission)
        {
            $response[$administrator_permission->{"administrator_permission_key::key"}] = json_decode($administrator_permission->{"administrator_permission::permission"});
        }

        return $response;
    }

    function saveAdministratorPermission($administrator_id, $key, $permission)
    {
        $this->load->model("administrator_permission_key_model");

        $model_filter = new stdClass();
        $model_filter->where["key"] = $key;
        $model_filter->get_first = true;
        $administrator_permission_key = $this->administrator_permission_key_model->search($model_filter);

        $model_filter = new stdClass();
        $model_filter->where["administrator_permission_key_id"] = $administrator_permission_key->administrator_permission_key_id;
        $model_filter->where["administrator_id"] = $administrator_id;
        $model_filter->get_first = true;
        $administrator_permission = $this->search($model_filter);

        if($administrator_permission != null)
        {
            $administrator_permission->permission = json_encode($permission);
            $this->save($administrator_permission);
        }
        else
        {
            $administrator_permission = new stdClass();
            $administrator_permission->administrator_permission_key_id = $administrator_permission_key->administrator_permission_key_id;
            $administrator_permission->administrator_id = $administrator_id;
            $administrator_permission->permission = json_encode($permission);
            $this->save($administrator_permission);
        }
    }
     ...
     **           EXTERNAL USAGE FUNCTION
     ** ========================================================================================= **/
    function getAdministratorGroupPermissionsByAdministratorGroupId($administrator_group_id)
    {
        $model_filter = new stdClass();
        $model_filter->where["administrator_group.administrator_group_id"] = $administrator_group_id;
        $model_filter->join = array(
            "administrator_group" => "administrator_group_id",
            "administrator_group_permission_key" => "administrator_group_permission_key_id"
        );
        $administrator_group_permissions = $this->search($model_filter);

        $response = array();
        foreach($administrator_group_permissions as $administrator_group_permission)
        {
            $response[$administrator_group_permission->{"administrator_group_permission_key::key"}] = json_decode($administrator_group_permission->{"administrator_group_permission::permission"});
        }

        return $response;
    }

    function saveAdministratorGroupPermission($administrator_group_id, $key, $permission)
    {
        ...
        $model_filter->where["key"] = $key;
        $model_filter->get_first = true;
        $administrator_group_permission_key = $this->administrator_group_permission_key_model->search($model_filter);

        $model_filter = new stdClass();
        $model_filter->where["administrator_group_permission_key_id"] = $administrator_group_permission_key->administrator_group_permission_key_id;
        $model_filter->where["administrator_group_id"] = $administrator_group_id;
        $model_filter->get_first = true;
        $administrator_group_permission = $this->search($model_filter);

        if($administrator_group_permission != null)
        {
            $administrator_group_permission->permission = json_encode($permission);
            $this->save($administrator_group_permission);
        }
        else
        {
            $administrator_group_permission = new stdClass();
            $administrator_group_permission->administrator_group_permission_key_id = $administrator_group_permission_key->administrator_group_permission_key_id;
            $administrator_group_permission->administrator_group_id = $administrator_group_id;
            $administrator_group_permission->permission = json_encode($permission);
            $this->save($administrator_group_permission);
        }
    }
